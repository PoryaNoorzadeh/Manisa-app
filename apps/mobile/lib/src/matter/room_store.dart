...
final class RoomCatalog {
  ...
  RoomCatalog moveRoom(String roomId, int offset) {
    final currentIndex = rooms.indexWhere((room) => room.id == roomId);
    if (currentIndex < 0) {
      throw ArgumentError.value(roomId, 'roomId', 'unknown room');
    }
    final targetIndex = currentIndex + offset;
    if (targetIndex < 0 || targetIndex >= rooms.length || offset == 0) {
      return this;
    }
    final reordered = List<ManisaRoom>.of(rooms);
    final room = reordered.removeAt(currentIndex);
    reordered.insert(targetIndex, room);
    return RoomCatalog(
      rooms: List<ManisaRoom>.unmodifiable(reordered),
      deviceRooms: deviceRooms,
    );
  }
