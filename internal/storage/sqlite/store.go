...
func (s *Store) CreateHome(ctx context.Context, home domain.Home) error {
	...
	if err != nil { return fmt.Errorf("create home: %w", err) }
	return nil
}
...
func (s *Store) ListHomes(ctx context.Context) ([]domain.Home, error) {
	...
	if err != nil { return nil, fmt.Errorf("list homes: %w", err) }
	defer rows.Close()
	var out []domain.Home
	for rows.Next() {
		var h domain.Home
		var created, updated string
		if err := rows.Scan(&h.ID, &h.Name, &created, &updated); err != nil { return nil, err }
...
func (s *Store) ListRooms(ctx context.Context, homeID string) ([]domain.Room, error) {
	...
	for rows.Next() {
		var r domain.Room
		var created, updated string
		if err := rows.Scan(&r.ID, &r.HomeID, &r.Name, &created, &updated); err != nil { return nil, err }
...
func (s *Store) CreateDevice(ctx context.Context, device domain.Device) error {
	...
	if err != nil { return fmt.Errorf("create device: %w", err) }
	return nil
}
