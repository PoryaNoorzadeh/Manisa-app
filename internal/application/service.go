...
func New(store domain.Store, matterController matter.Controller) *Service { return &Service{store: store, matter: matterController} }
...
func (s *Service) ListHomes(ctx context.Context) ([]domain.Home, error) { return s.store.ListHomes(ctx) }
...
func (s *Service) CommissionDevice(ctx context.Context, input CommissionDeviceInput) (domain.Device, error) {
	if s.matter == nil { return domain.Device{}, fmt.Errorf("matter controller unavailable") }
	...
	if input.HomeID == "" || input.Name == "" || input.ProductType == "" || input.SetupPayload == "" { return domain.Device{}, ErrInvalidInput }
	if input.Transport != domain.TransportMatterWiFi && input.Transport != domain.TransportMatterThread { return domain.Device{}, ErrInvalidInput }
	if input.Transport == domain.TransportMatterWiFi && input.WiFiSSID == "" && !input.NetworkOnly { return domain.Device{}, ErrInvalidInput }
	if input.Transport == domain.TransportMatterThread && input.ThreadDataset == "" && !input.NetworkOnly { return domain.Device{}, ErrInvalidInput }

	result, err := s.matter.Commission(ctx, matter.CommissionRequest{SetupPayload: input.SetupPayload, WiFiSSID: input.WiFiSSID, WiFiPassword: input.WiFiPassword, ThreadDataset: input.ThreadDataset, NetworkOnly: input.NetworkOnly})
	if err != nil { return domain.Device{}, fmt.Errorf("commission matter device: %w", err) }
...
func (s *Service) ExecuteDeviceCommand(ctx context.Context, deviceID string, input DeviceCommandInput) error {
	...
	if deviceID == "" || input.Endpoint == 0 || input.Capability == "" || input.Action == "" { return ErrInvalidInput }
	if s.matter == nil { return fmt.Errorf("matter controller unavailable") }
	device, err := s.store.GetDevice(ctx, deviceID)
	if err != nil { return err }
	if device.ExternalNodeID == nil || strings.TrimSpace(*device.ExternalNodeID) == "" { return ErrDeviceNotCommissioned }
	command, err := mapDeviceCommand(input)
	if err != nil { return err }
	if err := s.matter.Execute(ctx, matter.NodeID(*device.ExternalNodeID), command); err != nil { return fmt.Errorf("execute matter command: %w", err) }
	return nil
}
...
func (s *Service) createDevice(ctx context.Context, homeID, name, productType, transport string, roomID *string, externalNodeID *string) (domain.Device, error) {
	...
	now := time.Now().UTC()
	device := domain.Device{ID: id, HomeID: homeID, RoomID: roomID, Name: name, ProductType: productType, Transport: transport, ExternalNodeID: externalNodeID, CreatedAt: now, UpdatedAt: now}
...
func mapDeviceCommand(input DeviceCommandInput) (matter.Command, error) {
	...
	case "level_control":
		if input.Action != "set_level" { return matter.Command{}, ErrUnsupportedCapability }
		level, ok := numericParam(payload, "level")
		if !ok || level < 0 || level > 254 { return matter.Command{}, ErrInvalidInput }
		return matter.Command{Endpoint: input.Endpoint, ClusterID: 8, Name: "moveToLevelWithOnOff", Payload: payload}, nil
	default:
		return matter.Command{}, ErrUnsupportedCapability
	}
}
